...
function Admin_list_dog() {
    ...
    useEffect (()=>{
		...
		if (sessionStorage.getItem('admin_api_token')==null) {
			...
			axios.get('http://localhost/api/v4/list_dog?api_token='+sessionStorage.getItem('admin_api_token'),
			).then (
				res=> {	
						...
						for(let i = 0;i < res.data.length;i++){
							item_list[i] = [  res.data[i].pet_name, res.data[i].pet_birthday, res.data[i].pet_sex ,res.data[i].pet_weight , res.data[i].pet_vaccine, res.data[i].pet_bloodtype]; 
						}
...
<div>
          ...
          <div className = "kh2">
                    ...
                    เพศ :  {eventItem[2]}
					<br></br>
					น้ำหนัก :   {eventItem[3]}  kg
					<br></br>
					วัคซีน :   {eventItem[4]}
					<br></br>
					กรุ๊ปเลือด :   {eventItem[5]} 
					<br></br>
